...
(async () => {
  ...
  try {
    ...
    // --- audit tampering detected (last; mutates temp store + restarts gateway) ---
    {
      const receiptsPath = path.join(tmp, 'audit_receipts.json');
      const before = await admin.auditVerify(org_id);
      ok(before.ok, 'chain ok before tamper test');
      const gwPort = String(new URL(BASE).port);
      const spawnGateway = () => {
        const g = cp.spawn(process.execPath, [path.join(__dirname, '..', 'src', 'server.js')], {
          env: { ...process.env, PORT: gwPort, AUTHRA_DATA: tmp }, stdio: ['ignore', 'pipe', 'pipe'],
        });
        g.stdout.on('data', d => { if (process.env.AUTHRA_TEST_VERBOSE) process.stdout.write('[gw] ' + d); });
        g.stderr.on('data', d => { if (process.env.AUTHRA_TEST_VERBOSE) process.stderr.write('[gw-err] ' + d); });
        return g;
      };
      const stopGateway = async (g) => { try { g.kill('SIGTERM'); } catch {} await sleep(500); };
      try {
        const stored = JSON.parse(fs.readFileSync(receiptsPath, 'utf8'));
        const ids = Object.keys(stored);
        if (ids.length > 1) {
          stored[ids[ids.length - 1]].action = 'tampered.action';
          fs.writeFileSync(receiptsPath, JSON.stringify(stored, null, 2));
      ...
      try {
        const backup = fs.readFileSync(receiptsPath, 'utf8');
        await stopGateway(child);
        child = null;
        fs.writeFileSync(receiptsPath, 'not-json\n');
        const badChild = spawnGateway();
        let healthy = false;
        for (let i = 0; i < 8; i++) {
          try { const r = await fetch(base + '/v1/health'); if (r.ok) { healthy = true; break; } } catch {}
          await sleep(250);
        }
        try { badChild.kill('SIGTERM'); } catch {}
        await sleep(500);
        fs.writeFileSync(receiptsPath, backup);
        child = spawnGateway();
        await waitHealth(base);
        ok(healthy === false, 'corrupted audit syntax fails closed');
